...
class Signal(object):
    ...
    def to_wav(self, filename):
        ...
        """
        wavfile.write(filename, self.sample_frequency, self._data/np.abs(self._data).max() *  0.5)
        #wavfile.write(filename, self.sample_frequency, np.int16(self._data/(np.abs(self._data).max()) * 32767) )
